...
namespace flow
{
    ...
    public:
        ...
        TimestampNs getRateLimitWindow() const
        {
            return m_settings.rateLimitWindow;
        }

        int getRateLimitCount() const
        {
            return m_settings.rateLimitCount;
        }
    ...
    private:
        ...
        public:
            void process(const Order &order)
            {
                ...
                {
                    auto now = m_service.getEventDispatcher().getEventTime();
                    m_lastOrderSendTime = now;
                    m_service.addOrderSendTime(now);
            ...
            bool validateOrder(const Order &order) const
            {
                ...
                return m_book &&
                       order.tif == TIF::IOC &&
                       order.qty > 0 &&
                       now - m_lastOrderSendTime >= m_service.getMinOrderGap() &&
                       m_service.validateOrderRateLimit(now);
            }
    ...
    private:
        ...
        void addOrderSendTime(TimestampNs orderSendTime)
        {
            m_orderSendTimes.push(orderSendTime);
        }

        bool validateOrderRateLimit(TimestampNs now)
        {
            while (!m_orderSendTimes.empty() && now - m_orderSendTimes.front() >= getRateLimitWindow()) {
                m_orderSendTimes.pop();
            }
            return m_orderSendTimes.size() < getRateLimitCount();
        }
    ...
    private:
        ...
        std::queue<TimestampNs> m_orderSendTimes;
