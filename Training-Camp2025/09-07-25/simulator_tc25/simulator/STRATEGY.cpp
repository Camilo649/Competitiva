...
{
public:
std::map<std::string_view,vector<double>> data;
	std::map<std::string_view,double> prom;
	std::map<std::string_view,bool> has;
	std::map<std::string_view,bool> tene;
...
private:
     ...
     */
    void onTopOfBook(std::string_view s, const md::TopOfBook &book)
    {
        /// TODO - add logic
		/*std::cout<<"\nSymbol "<<s<<'\n';
		std::cout<<(book).bidSize<<'\n';
		std::cout<<(book).bidPrice<<'\n';
		std::cout<<(book).askSize<<'\n';
		std::cout<<(book).askPrice<<'\n';*/
		//if(s!="USD/JPY") return;
		
		data[s].push_back(book.askPrice);
		
		//cout<<book.bidPrice<<" "<<prom[s]<<'\n';
		if(prom[s]>=book.bidPrice && !tene[s]){
			//std::cout<<"compramo wacho\n";
			sendOrder((string)s,Side::Buy,book.bidPrice,1);
			tene[s]=true;
		}
		
		if(has[s] && book.askPrice>=prom[s] && tene[s]){
			//std::cout<<"vendemo wacho\n";
			sendOrder((string)s,Side::Sell,book.askPrice,1);
			tene[s]=true;
		}
    }
...
private:
    ...
    void onMinute()
    {
        ...
        // TODO - add logic
		for(auto i:data){
			if((int)i.second.size()>100 && !has[i.first]){
				//std::cout<<"tenemos la base wacho\n";
				double promedio = 0;
				for(auto j:i.second){
					promedio+=j;
				}
				promedio/=(int)i.second.size();
				has[i.first]=true;
				prom[i.first]=promedio;
				//tene[i.first]=false;
				//cout<<i.first<<" "<<prom[i.first]<<'\n';
				//cout<<promedio<<'\n';
			}
		}
    }
