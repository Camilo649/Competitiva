...
public:
    ...
    void onTopOfBook(std::string_view, const md::TopOfBook &book)
    {
        if (m_position < 0.01)
        {
            if ((book.askPrice - book.bidPrice) / book.bidPrice < MIN_ENTRY_SPREAD)
            {
                m_entryPrice = book.askPrice;
                sendOrder(Side::Buy, m_entryPrice, TARGET_POSITION - m_position);
            }
        }
        else
        {
            auto takeProfit = (book.bidPrice - m_entryPrice) / m_entryPrice > TAKE_PROFIT_THRESHOLD;
            auto stopLoss = (book.bidPrice - m_entryPrice) / m_entryPrice < STOP_LOSS_THRESHOLD;
            if (takeProfit || stopLoss)
            {
                sendOrder(Side::Sell, book.bidPrice, m_position);
            }
        }
    }
...
public:
    ...
    {
        // Logs positions
        std::cerr << m_eventDispatcher.getEventTime().count() << ",positions";
...
private:
    ...
    {
        // Logs PnL
        std::cerr << m_eventDispatcher.getEventTime().count() << ",pnl," << m_risk.getPnL(m_positions) << std::endl;
